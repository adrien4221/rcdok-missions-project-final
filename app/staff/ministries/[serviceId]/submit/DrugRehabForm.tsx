...
import { submitMinistryActivity } from '@/app/actions/actions'; // Verified absolute import path
import { CheckCircle2, AlertCircle } from 'lucide-react';
import { useSearchParams } from 'next/navigation';

type Parish = { id: string; name: string };

interface DrugRehabFormProps {
  serviceId: string;
  parishes: Parish[];
  initialData?: any;
  onSuccess?: () => void;
...
export default function DrugRehabForm({
  serviceId,
  parishes = [],
  initialData,
  onSuccess,
  ...
  // Hydrate form fields if editing a record
  useEffect(() => {
    ...
    setParishId(initialData.organizationId || '');

    setClientName(details.client_name || '');
    setDob(details.dob || '');
    setContactNumber(details.contact_number || '');
    setPrimarySubstance(details.primary_substance || '');
    setReferralSource(details.referral_source || 'Walk-in');
    setPriorityLevel(details.priority_level || 'Medium');
    setSessionType(details.session_type || 'Initial Intake');
    setAssignedProgram(details.assigned_program || 'Outpatient');
    setAssignedCounselor(details.assigned_counselor || '');
    setSessionNotes(details.session_notes || '');
  }, [initialData]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const result = await submitMinistryActivity({
      activityId: initialData?.id, // Injects ID on updates
      serviceId,
      parishId,
      activityDate,
      details: jsonPayload,
    ...
    if (result.success) {
      if (onSuccess) {
        ...
        alert(
          initialData
            ? 'Rehab Session Updated Successfully!'
            : 'Rehab Session Successfully Logged!'
        );
        ...
        setAssignedProgram('Outpatient');
        setAssignedCounselor('');
        setSessionNotes('');
      }
    } else {
      alert(result.message || 'An error occurred while logging the encounter.');
    }
  };
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-10">
      ...
      <div className="space-y-5">
        <h3 className="text-lg font-bold text-[#0060AF] border-b pb-2">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div>
            ...
            <input
              ...
              className="w-full px-4 py-2.5 rounded-lg border border-gray-300 focus:ring-2 focus:ring-[#0060AF] outline-none"
            />
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">
              Primary Substance of Dependency
            </label>
            <input
              required
              type="text"
              placeholder="e.g. Methamphetamine, Alcohol, Marijuana"
      ...
      <div className="space-y-5">
        <h3 className="text-lg font-bold text-[#0060AF] border-b pb-2">
          3. Session & Program Logistics
        </h3>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">
              Ministry Station (Parish)
            </label>
            <select
              required
              value={parishId}
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">
              Assigned Counselor / Facilitator
            </label>
            <input
              required
              type="text"
              placeholder="e.g. Dr. Santos or Bro. Mark"
          ...
          <div className="md:col-span-2">
            ...
            <textarea
              rows={4}
              placeholder="Summarize the session, client's progress, or next steps..."
              value={sessionNotes}
              onChange={(e) => setSessionNotes(e.target.value)}
              className="w-full px-4 py-2.5 rounded-lg border border-gray-300 focus:ring-2 focus:ring-[#0060AF] outline-none resize-none"
            />
          </div>
        </div>
      </div>
      ...
      <div className="pt-6 border-t">
        <button
          ...
          className="w-full md:w-auto bg-[#2dd4bf] hover:bg-[#14b8a6] text-black font-bold px-10 py-3 rounded-full shadow-md transition-transform hover:scale-105 active:scale-95 flex items-center justify-center gap-2 disabled:opacity-50 disabled:transform-none"
        >
          {isSubmitting
            ? initialData
              ? 'Updating Session...'
