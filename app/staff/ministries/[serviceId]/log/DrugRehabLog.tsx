...
import DrugRehabForm from '../submit/DrugRehabForm'; // Matches your project file structure convention
...
export default function DrugRehabLog({
  initialActivities,
  serviceId,
  parishes,
}: {
  ...
  // React state synchronization with Server Component Prop updates
  useEffect(() => {
  ...
  // Comprehensive multi-field text filtering
  ...
  const handleDelete = async (id: string) => {
    if (confirm('Are you sure you want to delete this rehabilitation encounter?')) {
  ...
  const exportCSV = () => {
    ...
    const rows = filteredActivities.map((activity) => {
      ...
      return {
        ...
        DateOfBirth: details?.dob ? new Date(details.dob).toLocaleDateString() : '',
        ContactNumber: details?.contact_number || '',
        PrimarySubstance: details?.primary_substance || '',
        ReferralSource: details?.referral_source || '',
        RiskLevel: details?.priority_level || 'Low',
        SessionNotes: details?.session_notes || '',
        AssignedCounselor: details?.assigned_counselor || '',
        AssignedProgram: details?.assigned_program || '',
    ...
    a.href = url;
    a.download = 'drug-rehabilitation-log.csv';
    a.click();
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-gray-500">
            A clinical record of all rehabilitation sessions, intakes, and graduations.
          </p>
        </div>
      ...
      <div className="text-sm text-gray-500">
        Showing {filteredActivities.length} encounter(s)
      </div>
        ...
        {activities.length === 0 ? (
          <div className="p-12 text-center text-gray-500 flex flex-col items-center">
            ...
            <p className="text-sm mt-1">
              Log a new session from the 'Submit Activity' tab to build the history.
            </p>
          </div>
        ) : (
                      ...
                      <td className="px-6 py-4 align-top">
                        ...
                        <div className="text-xs font-medium text-gray-700">
                          ...
                          <span className="text-red-600 font-semibold">
                            {details?.primary_substance || 'Unknown'}
                          </span>
                        </div>
                      </td>

                      {/* Encounter Details Column */}
                        ...
                        <div className="text-gray-500 text-xs flex flex-col gap-1">
                          ...
                          <span className="font-semibold text-gray-700">
                            ...
                            <span className="text-indigo-600">
                              {details?.assigned_program || 'Pending'}
                            </span>
                          </span>
                        </div>
                      </td>

                      {/* Row Modification Controls */}
      ...
      {/* Embedded Operational Edit Modal Overlay */}
      {editingActivity && (
